...
The skin setting has, besides the tones and the glowing web alone, an **Hologramme** option: a solid skin (the light tone tinted a little by the web's colour), the fine web of nodes and lines drawn over it, no hair, and the brows and the lip line in the web's colour. The web itself is denser than before (nodes 0.031 apart in head half-heights, computed when a face loads, with a grid for the neighbour search). The skin of the hologram is solid: brighter than the plain looks, with a faint cool light at the contour only, opaque down to the base of the neck (it used to melt into the background), and only the brighter web nodes at the contour are drawn, not a dotted veil over the face.

**Hologramme bleu** (skin setting): the same hologram with a solid blue skin (a fifth skin tone, lit like the others) instead of the matt one.
