...
Setting Avatar > Casquette: none, black, blue, red, white, khaki (`avatar/CapGeometry.kt`, drawn by `AvatarRenderer.drawCap`). The dome is the top of the head's skin pushed out along the normals, rebuilt every frame from the posed head, so it follows every movement; its edge is a curve (level with the top of the ears, just above the brows, lower at the back) and the points just below it are lifted onto it for a clean edge. A visor runs from the front edge, forward and a little down, longest in the middle; six seams and a button. The hair under the cap is not drawn (locks and their fibres included); the hair below it stays. It works with every skin look, the hologram ones included. Not for the cartoon face. Checked on the emulator only (front view and a moving head).
